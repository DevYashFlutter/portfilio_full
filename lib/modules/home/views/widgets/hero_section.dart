    ...
    return Container(
      ...
      child: Stack(
        ...
        children: [
          ...
          // 4. Scroll Indicator
          const Positioned(bottom: 32, left: 0, right: 0, child: _HeroScrollIndicator()),
        ],
      ),
    );
  }
    ...
    return Stack(
      children: [
        ...
        Positioned(
          ...
          child: _Orb(color: const Color(0xFFA855F7).withValues(alpha: 0.15), size: 350, duration: 8),
        ),
        // Cyan Orb
        Positioned(
          bottom: 0.25 * 900,
          right: 0.15 * MediaQuery.of(context).size.width,
          child: _Orb(color: const Color(0xFF06B6D4).withValues(alpha: 0.15), size: 350, duration: 8, reverse: true),
        ),
        // Blue Orb
        Positioned(
          top: 0.45 * 900,
          right: 0.33 * MediaQuery.of(context).size.width,
          child: _Orb(color: const Color(0xFF3B82F6).withValues(alpha: 0.15), size: 280, duration: 6),
        ),
      ],
    );
  }
...
class _Orb extends StatelessWidget {
  ...
  const _Orb({required this.color, required this.size, required this.duration, this.reverse = false});
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        .custom(
          builder: (context, value, child) => Opacity(opacity: reverse ? (0.5 - (value * 0.2)) : (0.3 + (value * 0.2)), child: child),
        )
...
class _FloatingCard extends StatelessWidget {
  ...
  const _FloatingCard({required this.left, required this.top, required this.delay, required this.duration, required this.dx, required this.dy});
...
class _HeroBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [const Color(0xFFA855F7).withValues(alpha: 0.2), const Color(0xFF06B6D4).withValues(alpha: 0.2)]),
...
class _HeroHeadline extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
          children: [
            ShaderMask(
              shaderCallback: (bounds) => const LinearGradient(colors: [Colors.white, Color(0xFFE9D5FF), Color(0xFFCFFAFE)]).createShader(bounds),
              child: Text(
                'Flutter Developer',
                textAlign: TextAlign.center,
                style: TextStyle(fontSize: fontSize, fontWeight: FontWeight.bold, height: 1.1, color: Colors.white),
              ),
            ),
            ShaderMask(
...
class _HeroSubtitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Text(
          '2.6+ years crafting production-ready mobile applications for millions of users. Specialized in cross-platform development with Flutter, '
          'building scalable architectures and optimizing performance.',
          textAlign: TextAlign.center,
          style: TextStyle(
            fontSize: 18,
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [BoxShadow(color: const Color(0xFFA855F7).withValues(alpha: 0.25), blurRadius: 20, offset: const Offset(0, 4))],
              ),
...
class _HeroTechBadges extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final tech = ['Flutter', 'Firebase', 'SharedPreference', 'SQLite', 'Hive', 'REST APIs', 'GitHub Actions', 'GetX', 'MVC', 'Clean'];
    ...
    return Column(
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(30),
                border: Border.all(color: const Color(0xFFA855F7).withValues(alpha: 0.3), width: 2),
              ),
              child: Stack(
                ...
                children: [
                  Container(
                    ...
                    width: 6,
                    height: 6,
                    decoration: const BoxDecoration(color: Color(0xFFC084FC), shape: BoxShape.circle),
                  ).animate(onPlay: (c) => c.repeat()).moveY(begin: 0, end: 20, duration: const Duration(seconds: 2), curve: Curves.easeInOut),
                ],
              ),
            ),
          ],
